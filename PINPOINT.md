**Pinpoint game**

Pinpoint is a word-guessing game. The player is shown a sequence of clues, 
one at a time, and tries to guess the category/connection that they all share. 
The fewer clues needed, the better the score and there are up to 5 clues total, with a guess allowed after each one.

****User Stories****

* As a user, I want to access Pinpoint from the GameHive homepage, the same way I access other games.

* As a user, I want to start a game of Pinpoint and to be provided a first clue and then be able to input my guess, 
and continue doing that until the fifth clue which will be the last one.

* As a user, I want to see an error if I submit a blank guess, or other invalid values. 

* As a user, I want to see how many clues I've revealed and how many guesses I have left, 
so I know where I stand.

* As a user, after every wrong guess of mine, I want another clue to be revealed unless
I reached the final clue. 

* As a user, if my input matched the answer, I want to be able to win the game

* As a user, if I reached my fifth guess and it was wrong I want the game to end with a loss. 

* As a user, when I win, I want to see a success message showing how many clues it took me.

* As a user, when I lose, I want to see the correct answer revealed.

## Stretch Goals User Stories

* As a user, I want to be able to play the gave several times and not 
repeate the same game.

* As a user, I want to see a leaderboard of high scores.

## **MVP Goals**

A single hardcoded (or DB seesed) game, playable to a logged in user, with no result input provided yet (e.g."Guessed in one try!"). 
The goal is a fully working core game loop before adding game variety and user scores.

## Additional Goals
* game is not repeatable mechanism
* variety of at least 10 games
* high score functionality

For the MVP, the game just plays out in memory. A game object keeps track of how many guesses have been made and whether
the player has won, lost, or is still in progress.

When the game ends, that result gets shown on the page and then forgotten as nothing is saved.

When I add the "guessed in X tries" feature later, I don't need to change how the game itself works,
since it already knows everything needed (how many guesses were made, and whether it was won or lost).

The only change is:
right after a guess is made, I'll check whether that guess just ended the game. If it did, I'll take the game's
own data (win/lose, guesses used) along with whoever is logged in, and save that as one row in the Game_history table.
Everything else about how the game is played stays exactly the same — I'm just adding one extra step at the very end.

**Goals broken down to phases:**

**First part (MVP):** 

I'll build one working game that anyone logged in can play. I'll show a clue, take a guess, reveal more clues if wrong,
and end the game with a win or a loss after 5 tries( or if guessed previously whin then. I won't save anything anywhere;
the result will just show on the screen and then it's gone.

**Middle part (next phase):** 
I'll add a table that remembers what happened( who played, which puzzle, whether they won, and how many guesses it took).
Right when a game ends, I'll save one row with that info. 
Then I'll be able to actually show "guessed in 3 tries!", since it's been recorded.

**Stretch goals (later, optional):** 
Using that saved history, I'll stop showing someone a puzzle they've already played, and I'll build more puzzles 
(around 10) so there's real variety instead of repeats. 
I'll also build a leaderboard page that reads from that same saved history to show top scores.


# **DB tables:**

**pinpoint_games table:**

ID:ID primary key
Answer: varchar   - the hidden answwers/connections e.g. "things that are red" 

**Clues Table:**

id:primary key       -autogenerated
pinpoint_game_id: FK        - to know which game this belongs to (references pinpoint_games.id)
clue_text: varchar   - actual clues
clue_order: int      - order of clues from vague to easier

**Game_history table:**
id: bigint , Primarykey       - autogenerated
user_id bigint, FK            -who played
game_id bigint, FK            -which game was played             
won: boolean (true/false)     - did they win/loose
Guesses_used: int             -how many guesses have they had

**Note for later**
Game already tracks guessesMade and status internally, so no changes are needed there. 
The save happens in GameService's guess-handling method: after calling game.submitGuess(...), 
check if game.getStatus() is no longer IN_PROGRESS — if so, build a GameHistory 
record from the game's own data plus the logged-in user, and save it via GameHistoryRepository. 
The controller only needs to start passing the logged in user into that method

TO DO
create tables, one game in there, tdd when I create a game object etc. 

****## Testing****

**Testing start of game:**

* A new game starts with 1 clue revealed
* A new game starts with 0 guesses made
* A new game starts with starts in_progress

**Testing a submission of a correct guess:**

* A correct guess sets status to Won
* A correct guess is possible without being sensitive to formating (e.b. Blue/blue/BLUE will be accepted)
* A correct guess returns something like GuessResult.CORRECT

**Submitting incorrect guess**

* An inccorrect guess increases NumberOfCluesRevealed by 1
* An incorrect guess increases guessesMade by 1
* An incorrect guess keeps status as IN_PROGRESS
* An incorrect guess returns GuessResult.INCORRECT


**Submitting an Incorrect guess on the final (5th) try**

Status changes to LOST
No further clue is given
Returns GuessResult.GAME_OVER

**Trying to guess when the game ended**
* Throws an exception rather than doing nothing quietly
* the game's status and guess count remain unchanged 

**Tests for Clues**

* A game can be saved and retrieved with its clue attached
* Clues come back in the correct order by sorting clue_order


**validation/edge-case**

* An incorrect blank guess 
* Guess containing only numbers is rejected
* Guess containing special characters is rejected
* Rejects a guess if the game has already ended

**Stretch goal test:
GameService Tests:**

* When a game ends with a win, GameHistory record is saved with won=true and the correct guessesUsed
* When a game ends in a loss, GameHistory record is saved with won=false
* No gameHistory record is saved while the game is still IN_PROGRESS












